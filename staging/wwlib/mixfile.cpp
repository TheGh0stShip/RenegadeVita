...
MixFileFactoryClass::MixFileFactoryClass( const char * mix_filename, FileFactoryClass * factory )	:
	...
	IsModified (false)
{
	static_assert(sizeof(MIXFILE_HEADER) == 12, "MIX1 header must use 32-bit fields");
	static_assert(sizeof(FileInfoStruct) == 12, "MIX1 file entry must use 32-bit fields");
...
**
*/
#if !defined(RENEGADE_RUNTIME_READ_ONLY)
