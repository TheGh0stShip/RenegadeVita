...
//		if (1 || Is_Game_Allowed( find_info.cFileName ) ) {
		if ( Is_Game_Allowed( find_info.cFileName ) ) {
			...
			if (item_index >= 0) {
				...
				list_ctrl->Set_Entry_Data (item_index, 1, Renegade_Ui_Pointer_To_Token(new StringClass(file_path)));
...
)
{
	if (ctrl_id == IDC_LOAD_GAME_LIST_CTRL) {
		...
		StringClass *path			= static_cast<StringClass *>(Renegade_Ui_Take_Pointer_Token(list_ctrl->Get_Entry_Data (item_index, 1)) );
...
LoadSPGameMenuClass::Update_Button_State (void)
{
	...
	if (item_index != -1) {
		...
		//		
		if (list_ctrl->Get_Entry_Data (item_index, 0) != 0U) {
			StringClass filename = (static_cast<StringClass *>(Renegade_Ui_Pointer_From_Token(list_ctrl->Get_Entry_Data (item_index, 1)) ))->Peek_Buffer ();
