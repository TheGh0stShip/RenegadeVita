...
| v3.5 | Correctness and flight recorder: repair A3.2 defects; matching diagnostic candidate | `A3.5-dev87` is retained physical frontend-usability failure evidence: menu text remained absent, intro A/V was slow/buzzy, and the original dialogue box was empty. `A3.5-dev88` is published local-only history. `A3.5-dev89` is the current local-only canonical follow-up: it adds frontend 800×600 scoping, font fallback/glyph validation, bootstrap display flushing, expanded UI/HUD/M00 pre-cache touches, Render2D texture-state isolation, BINK late-frame dropping, and native-coordinate target-box scoping. Intro/menu/A-V, loading/HUD/text/shadow, START-exit, HMVV freeze, and FPS defects remain physically open. |
