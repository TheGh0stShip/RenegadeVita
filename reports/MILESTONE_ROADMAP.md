...
| v3.5 | Correctness and flight recorder: repair A3.2 defects; matching diagnostic candidate | `A3.5-dev87` is retained physical frontend-usability failure evidence: menu text remained absent, intro A/V was slow/buzzy, and the original dialogue box was empty. `A3.5-dev88` through `A3.5-dev91` are published local-only history. `A3.5-dev92` is the current local-only canonical follow-up: it corrects FreeType glyph cell/bearing placement, strengthens visible glyph-column probes, reduces BINK upload/update work with explicit audio-layout fallback, and adds target-box projection diagnostics before further math changes. Intro/menu A/V/text, loading/HUD/dialogue/pickup text, shadows/walls, START-exit, HMVV freeze, and FPS defects remain physically open. |
