...
| Latest local candidate | **A3.5-dev92**: canonical ARM/VPK validation passed after source-only corrections for FreeType glyph cell/bearing placement, visible glyph-column probes, lower-cost BINK frame uploads, explicit movie audio input layout fallback, and bounded target-box projection diagnostics. It has **not** been copied to, installed on, or launched on a Vita. |
| Visual evidence | The gallery contains reviewed historical frames only. A user-finalized Dev87 recording supplied six labelled M00 stills. Its full MP4/player is prepared on a private `gh-pages` branch; GitHub rejected public Pages for this account plan. The user has authorized unlisted YouTube hosting, pending connection of the upload-capable account. No Dev88, Dev89, Dev90, Dev91, or Dev92 physical frame exists. |
...
- [Current status](docs/CURRENT_STATUS.md) — accepted baseline, Dev87 result, Dev92 limits, and next physical evidence gate.
