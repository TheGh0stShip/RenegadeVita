...
`proto->Create()` took 6,122,860 us. Its class ID 25 maps to original HLOD.
...
Next: time `HLodClass(const HLodDefClass&)` before/after LOD child creation,
aggregates, static LOD factors, cost/value, and bounding-volume updates for
this model, then change only the confirmed hot operation.
