...
def parse(rc,d):
 ...
 while i<len(lines):
  ...
  while i<len(lines) and lines[i].strip()!='END':
   ...
   p=split(stmt); stmt=''
   if ident in IDS and p and (p[0].split(None,1)[0].upper() in CLASS or p[0].split(None,1)[0].upper()=='CONTROL'): body.append(control(p,d))
  if ident in IDS:
 ...
 return got
def main():
 a=argparse.ArgumentParser();a.add_argument('--rc',required=True);a.add_argument('--resource-h',required=True);a.add_argument('--dialog-resource-h',required=True);a.add_argument('--out',required=True);q=a.parse_args(); data=parse(q.rc,macros((q.resource_h,q.dialog_resource_h)))
