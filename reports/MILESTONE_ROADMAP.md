...
| v3.5 | Correctness and flight recorder: repair A3.2 defects; matching diagnostic candidate | `A3.5-dev87` is retained physical frontend-usability failure evidence: menu text remained absent, intro A/V was slow/buzzy, and the original dialogue box was empty. `A3.5-dev88` through `A3.5-dev96` are local-only history. `A3.5-dev97` is the current local-only canonical follow-up: it keeps Dev96's startup/loading/deferred Render2D/HUD/BINK/message-window/short-wchar fixes, adds bounded UTF-16 formatting for original formatted text, and makes the earliest startup-precache status/receipt candidate-scoped before retail MIX factory construction. Intro/menu A/V/text, loading/HUD/dialogue/pickup text, shadows/walls, START-exit, HMVV freeze, and FPS defects remain physically open. |
