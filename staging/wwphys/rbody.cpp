...
bool RigidBodyClass::Save (ChunkSaveClass &csave)
{
	...
	uint32 ode_ptr_token = static_cast<uint32>(
		reinterpret_cast<uintptr_t>((ODESystemClass *)this));
	csave.Begin_Chunk(RBODY_CHUNK_VARIABLES);
	WRITE_MICRO_CHUNK(csave,RBODY_VARIABLE_ODESYSTEM_PTR,ode_ptr_token);
...
bool RigidBodyClass::Load (ChunkLoadClass &cload)
{
			...
			case RBODY_CHUNK_VARIABLES:
				...
				while (cload.Open_Micro_Chunk()) {
					switch(cload.Cur_Micro_Chunk_ID()) {
						READ_MICRO_CHUNK(cload,RBODY_VARIABLE_ODESYSTEM_PTR,odesys_token);
	...
	if (odesys_token != 0) {
		SaveLoadSystemClass::Register_Pointer(
			reinterpret_cast<ODESystemClass *>(static_cast<uintptr_t>(odesys_token)),
			(ODESystemClass *)this);
	}
