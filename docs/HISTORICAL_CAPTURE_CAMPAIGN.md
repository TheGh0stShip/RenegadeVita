...
and every A3.5 candidate from dev1 through dev93. The archive therefore does
...
| Current/unvalidated local candidates | dev88–dev93 | Remain excluded until their separate physical test is explicitly authorized. |

The passes deliberately cover every retained historical A3.5 number from dev1
through dev87 exactly once. The row for dev88–dev93 stays local-only so the
