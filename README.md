...
| Latest local candidate | **A3.5-dev97**: canonical ARM/VPK validation passed after Dev96's UTF-16/wide-libc boundary work plus a bounded UTF-16 formatter and candidate-scoped early startup-precache status/receipt path. It has **not** been copied to, installed on, or launched on a Vita. |
| Visual evidence | The gallery contains reviewed historical frames only. A user-finalized Dev87 recording supplied six labelled M00 stills. Its full MP4/player is prepared on a private `gh-pages` branch; GitHub rejected public Pages for this account plan. The user authorized unlisted YouTube hosting, pending connection of the upload-capable account. No Dev88, Dev89, Dev90, Dev91, Dev92, Dev93, Dev94, Dev95, Dev96, or Dev97 physical frame exists. |
...
- [Current status](docs/CURRENT_STATUS.md) — accepted baseline, Dev87 result, Dev97 limits, and next physical evidence gate.
