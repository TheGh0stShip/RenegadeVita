...
| Latest local candidate | **A3.5-dev93**: canonical ARM/VPK validation passed after source-only corrections for early visible bootstrap status, Render2D text-atlas height/UV handling, original dialog-template/text diagnostics, and native 960x544 gameplay HUD/text presentation. It has **not** been copied to, installed on, or launched on a Vita. |
| Visual evidence | The gallery contains reviewed historical frames only. A user-finalized Dev87 recording supplied six labelled M00 stills. Its full MP4/player is prepared on a private `gh-pages` branch; GitHub rejected public Pages for this account plan. The user authorized unlisted YouTube hosting, pending connection of the upload-capable account. No Dev88, Dev89, Dev90, Dev91, Dev92, or Dev93 physical frame exists. |
...
- [Current status](docs/CURRENT_STATUS.md) — accepted baseline, Dev87 result, Dev93 limits, and next physical evidence gate.
