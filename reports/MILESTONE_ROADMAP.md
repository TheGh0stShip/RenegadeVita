...
| v3.5 | Correctness and flight recorder: repair A3.2 defects; matching diagnostic candidate | `A3.5-dev82` is the current source/build/package and manual physical-test candidate. It preserves the dev48-dev81 audio/texture/material/render-state chain and targets the latest physical defects: loading-screen coverage/text/progress, one-bar loading/prewarm, missing HUD/TextDisplay/subtitles, camera-Y and D-pad drift, action/use mapping, reload animation, sniper scope/icon/zoom, NPC/Havoc/door/powerup/objective texture orientation/materials, FPS state churn, gate interaction diagnostics, and freeze triage. Physical acceptance remains pending; soak remains v3.9 |
