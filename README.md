...
| Latest local candidate | **A3.5-dev94**: canonical ARM/VPK validation passed after source-only corrections for startup framebuffer handoff, synchronous loading-progress callbacks, deferred DX8 state before indexed Render2D text/HUD draws, HUD `Think()` presentation scoping, BINK audio-pressure frame dropping, and host main-menu translation validation. It has **not** been copied to, installed on, or launched on a Vita. |
| Visual evidence | The gallery contains reviewed historical frames only. A user-finalized Dev87 recording supplied six labelled M00 stills. Its full MP4/player is prepared on a private `gh-pages` branch; GitHub rejected public Pages for this account plan. The user authorized unlisted YouTube hosting, pending connection of the upload-capable account. No Dev88, Dev89, Dev90, Dev91, Dev92, Dev93, or Dev94 physical frame exists. |
...
- [Current status](docs/CURRENT_STATUS.md) — accepted baseline, Dev87 result, Dev94 limits, and next physical evidence gate.
