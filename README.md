...
| Latest local candidate | **A3.5-dev91**: canonical ARM/VPK validation passed after source-only corrections for startup visible-delay reduction, BINK upload/audio pacing, loading-progress catch-up frames, stricter visible-glyph probing, and M00 vehicle/HMVV proximity diagnostics. It has **not** been copied to, installed on, or launched on a Vita. |
| Visual evidence | The gallery contains reviewed historical frames only. A user-finalized Dev87 recording supplied six labelled M00 stills. Its full MP4/player is prepared on a private `gh-pages` branch; GitHub rejected public Pages for this account plan. The user has authorized unlisted YouTube hosting, pending connection of the upload-capable account. No Dev88, Dev89, Dev90, or Dev91 physical frame exists. |
...
- [Current status](docs/CURRENT_STATUS.md) — accepted baseline, Dev87 result, Dev91 limits, and next physical evidence gate.
