...
| v3.5 | Correctness and flight recorder: repair A3.2 defects; matching diagnostic candidate | `A3.5-dev87` is retained physical frontend-usability failure evidence: menu text remained absent, intro A/V was slow/buzzy, and the original dialogue box was empty. `A3.5-dev88` through `A3.5-dev90` are published local-only history. `A3.5-dev91` is the current local-only canonical follow-up: it reduces startup artificial holds, downscales unchanged BINK uploads with larger audio reserve and faster late-frame dropping, adds synchronous loading catch-up renders, strengthens visible glyph probes, and adds M00 vehicle/HMVV proximity diagnostics. Intro/menu A/V/text, loading/HUD/dialogue/pickup text, shadows/walls, START-exit, HMVV freeze, and FPS defects remain physically open. |
